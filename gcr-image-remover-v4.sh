...
echo "Images older than $AGE_THRESHOLD_MONTHS months in gcr.io/$PROJECT_ID/$REPOSITORY_NAME:"
gcloud container images list-tags gcr.io/$PROJECT_ID/$REPOSITORY_NAME --format="table(digest,tags,timestamp)" --filter="TIMESTAMP.datetime < '$THRESHOLD_DATE'"
...
echo "The number of images older than $AGE_THRESHOLD_MONTHS months in gcr.io/$PROJECT_ID/$REPOSITORY_NAME is: $IMAGE_COUNT"
